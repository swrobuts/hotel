...
KENNZAHLEN = """COUNT(*)                                             AS anzahl,
  ...
  COALESCE(SUM(f.revenue) FILTER (WHERE NOT f.is_canceled), 0) AS erloes_nicht_storniert,
...
def kennzahlen(filter: Filter) -> dict:
  ...
  AVG(f.adr)                                           AS adr,
  COALESCE(SUM(f.revenue), 0)                           AS gesamterloes,
  COALESCE(SUM(f.revenue) FILTER (WHERE NOT f.is_canceled), 0) AS erloes_nicht_storniert,
  COALESCE(SUM(f.revenue) FILTER (WHERE f.is_canceled), 0) AS erloes_storniert,
