...
function leadUeberblick(d) {
  ...
  return `${kurz(k.erloes_nicht_storniert)} EUR Umsatz aus ${zahl(k.anzahl_buchungen)} Buchungen, ${prozent(k.stornoquote)} storniert.`;
}
...
function aussagen(d) {
  ...
  return {
    hotelvergleich: [city && resort
      ...
      : "Hotelvergleich",
      beschreibung("Kennzahlen je Hotel und für beide Hotels zusammen; Umsatz = nicht stornierte Buchungen")],
    kennzahlen: [leer ? "Keine Buchungen im gewählten Ausschnitt" : `${zahl(k.anzahl_buchungen)} Buchungen, ${prozent(k.stornoquote)} storniert, ${dezimal(k.adr)} EUR mittlerer Zimmerpreis`,
    ...
    zeitverlauf: [spitze ? `${monatLang(spitze)} war der stärkste Anreisemonat: ${zahl(spitze.anzahl_buchungen)} Buchungen${erloesSpitze ? `; höchster Umsatz im ${monatLang(erloesSpitze)} (${kurz(erloesSpitze.erloes_nicht_storniert)} EUR)` : ""}` : "Kein Zeitverlauf verfügbar",
      beschreibung("Anzahl Buchungen, Umsatz in EUR (nicht stornierte Buchungen) und Stornoquote je Anreisemonat; Versatzstück = Abweichung zum Vorjahresmonat")],
    segment: [top ? `${klarname("segment", top.segment)}: ${anteil(top.anzahl, d3.sum(d.segmente, (z) => z.anzahl))} der Buchungen, ${anteil(top.erloes_nicht_storniert, d3.sum(d.segmente, (z) => z.erloes_nicht_storniert))} des Umsatzes${abweicher(d.segmente, "segment")}` : "Keine Buchungen",
      beschreibung("Je Marktsegment der Anteil an den Buchungen neben dem Anteil am Umsatz (nicht stornierte Buchungen), gleiche Skala, absolute Werte in Klammern")],
    kanal: [kanal ? `${klarname("kanal", kanal.kanal)}: ${anteil(kanal.erloes_nicht_storniert, d3.sum(d.kanaele, (z) => z.erloes_nicht_storniert))} des Umsatzes bei ${anteil(kanal.anzahl, d3.sum(d.kanaele, (z) => z.anzahl))} der Buchungen${abweicher(d.kanaele, "kanal")}` : "Keine Buchungen",
      beschreibung("Je Vertriebskanal der Anteil an den Buchungen neben dem Anteil am Umsatz (nicht stornierte Buchungen), gleiche Skala, absolute Werte in Klammern")],
    ...
    erloesverlust: [k.anzahl_buchungen ? `${kurz(k.erloes_storniert)} EUR (${anteil(k.erloes_storniert, k.gesamterloes)} des gebuchten Umsatzes) gingen durch Stornierungen verloren${verlust ? `, am meisten im ${monatLang(verlust)}` : ""}` : "Keine Daten",
      beschreibung("Gebuchter Umsatz je Anreisemonat in EUR, aufgeteilt in Umsatz (nicht storniert) und durch Stornierung entgangen")],
...
function deutungen(d) {
  ...
  T.hotelvergleich = !(city && resort) ? ["Für den Vergleich müssen beide Hotels Buchungen haben.", "Filter lockern."] : [
    ...
    + `das Resort Hotel hat längere Aufenthalte (${dezimal(resort.aufenthaltsdauer)} gegenüber ${dezimal(city.aufenthaltsdauer)} Nächte) und ${resort.adr > city.adr ? "den höheren" : "den niedrigeren"} Zimmerpreis (${dezimal(resort.adr)} gegenüber ${dezimal(city.adr)} EUR).`,
  ...
  T.kennzahlen = leer ? ["Keine Buchungen im gewählten Ausschnitt.", "Filter lockern."] : [
    ...
    `Ein Prozentpunkt weniger Stornierungen entspricht etwa ${zahl(k.gesamterloes / 100)} EUR gebuchtem Umsatz – die Stornoquote ist der größte Hebel. Der geringe Anteil an Wiederholungsgästen spricht für ein Bindungsprogramm.`,
  ];
